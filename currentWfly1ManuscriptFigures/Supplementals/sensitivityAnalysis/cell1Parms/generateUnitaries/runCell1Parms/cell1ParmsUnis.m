...
system('mkdir simulations/cell1Parms')

 %path to the dir containing the hoc files to be run
path1=['~/nC_projects/',PN,'_allORNs/simulations/cell1Parms/'];
cd(path1)
...
% into this dir
system('cp -a ../../../PN1LS_allORNs/simulations/cell1ParmsCondAdj/DoubExpSynA.mod ./')
...
gmax=1/(8.3*1000) ; %in uF*cm^-2
%multiplication of gmax here is simply to match the format in the neuron
%file, I hardcode the E-4
chngGmaxCmd_nrnFile=['sed -i -e ''s#gmax_LeakConductance = 4.8E-5#gmax_LeakConductance = ',num2str(gmax*10000,2),'E-4#'' ',neuronFileName];
...
system(['sed -i -e ''s#\/simulations\/#\/simulations\/cell1Parms\/#'' ', PN,'_allORNs.hoc'])
