...
%% calculate PN dendrite length within the glomerulus and number of sites postsynaptic to ORNs
...
% Step 6 find all sittes post-synaptic to ORNs
postORNSites=0;
...
for v=dendInds
    ...
    for c=1:length(children)
        ...
        if strcmp(workingSkel.connectivity.(cell2mat(skelVertNames(v))).(cell2mat(children(c))).type, 'presynaptic_to') == 1
            ...
            %Is the presynaptic neuron an ORN?
            if ismember(cell2mat(children(c)), connFields)== 1 && isempty(conns.((cell2mat(children(c)))).pre) ~= 1
            if ismember(conns.((cell2mat(children(c)))).pre, ORNs)
            
              postORNSites=postORNSites+1;
...
postORNSiteNum(s)=postORNSites;


end

dendrites{p}(1,:)=totalLength;
dendrites{p}(2,:)=postORNSiteNum;
...
save('PN_Dendrites', 'dendrites')


for i=1:5

        pathLs(i)=sum(dendrites{i}(1,:));
        ornIns(i)=sum(dendrites{i}(2,:));
end
