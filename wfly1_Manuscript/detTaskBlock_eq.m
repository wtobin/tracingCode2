...
for e=1:numel(activeSyns)
    
    if size(rem,1)>=numel(ipsiORNs)
        
        if counter<=numel(ipsiORNs)
            activeSyns(e,2)=counter;
            counter=counter=1;
            rem(e,:)=[]
        else
            counter=1;
    ...
    elseif endState==1
        remAssignments=randsample(numel(ipsiORNs),size(rem,1));
        activeSyns(e,2)=remassignments(finalCounter);
        endState=endState+1;
