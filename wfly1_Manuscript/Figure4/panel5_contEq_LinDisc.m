...
for p=1 %:numel(PNs)
    ...
    for t=1:2
        ...
        for rep=1:550
            ...
            % Find all synapses coming from ipsi ORNs
            
            if strcmp(PN, 'PN1RS') == 1 || strcmp(PN,'PN2RS') == 1 %For Right PNs
                
                ipsiConts=pullContactNums(ORNs_Right,path1,path2);
                
                
            else % For L PNs
                
                ipsiConts=pullContactNums(ORNs_Left,path1,path2);
           ...
           numCont=floor(numel(ipsiConts(:,1))/numel(unique(ipsiConts(:,2)))); % Num Contacts each ORN will get
                
           
            
           % Shuffle the ipsi contacts
           shuffIpsi=randsample(ipsiConts(:,1),numel(ipsiConts(:,1)));
                

            
            for i=1:numel(unique(ipsiConts(:,2)))
            ...
            if ctg(p,dFCount,t,rep) == 1
                ...
                for o=1:numel(unique(ipsiConts(:,2)))
                ...
                for o=1:numel(unique(ipsiConts(:,2)))
                    ...
                    spikeTrain(o,:)=[makeSpikes(.001,2.25,.099),makeSpikes(.001,(2.25+dF),.1)];
                    spikeTimes{o}=find(spikeTrain(o,:)==1);
            ...
            saveSpikeVectors(totSynapseNums,activeSyns_eq,spikeTimes,path2)
...
save('disrimSim_realCon_Cat_eq','ctg')
save('disrimSim_realCon_pnVms_eq','pnVms')
save('disrimSim_realCon_meanVms_eq','vmMeans')