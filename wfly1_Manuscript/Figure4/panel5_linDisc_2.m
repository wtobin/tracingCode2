...
for p=1 %:numel(PNs)
 ...
 for rep=1:50
   ...
   % Calculate the mean voltage for all reps
    vmMeans(p,t,:)=mean(squeeze(pnVms(p,t,:,:))');
...
%     
%    
% predictors=squeeze(vmMeans(1,1,:));
% categories=squeeze(ctg(1,1,:));
% 
...
% sum(categories(find(predictors>thresh))==2)/sum(categories==2)
%  
% %see how well it did on the test data
% 
% sum(categories(find(squeeze(vmMeans(1,2,:))<thresh))==1)/sum(squeeze(ctg(1,2,:))==1)
% sum(categories(find(squeeze(vmMeans(1,2,:))>thresh))==2)/sum(squeeze(ctg(1,2,:))==2)
%     
