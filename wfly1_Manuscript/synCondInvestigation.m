...
sequence2=importdata([resultDir,'/neuron_PN1_LS_sk_419138_0.dat']);
plot(sequence2+59.4)
grandSum(2)=sum(sequence2+59.4);

%Set the name of the directory to which the results will be saved
resultDir=['results_linearityTest/case6_redCondSim'];
mkdir(resultDir)
chngResDir=['sed -i -e ''s#{ sprint(targetDir, "%s%s/", simsDir, simReference)}#targetDir="',path1,resultDir,'/"#'' ',hocCpName];
system(chngResDir)
...
%import result vector
simultaneous2=importdata([resultDir,'/neuron_PN1_LS_sk_419138_0.dat']);
