function [] = saveSpikeVectors( totSynapseNums,activeSyns,spikeTimes, path )
%saveSpikeVectors
%{
...
path - path to the directory in which spike vector files will be saved
...
for f=1:numel(totSynapseNums)
    ...
    if ismember(s,activeSyns(:,1))
        ...
        if isempty(spikeTimes{activeSyns(find(activeSyns(:,1) == s),2)}) == 1
            ...
            vector=[];
            save([path,'/spikeVector',num2str(totSynapseNums(f)),'.txt'],'vector','-ascii')
