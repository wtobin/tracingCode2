...
for i=1:length(PNs)
    ...
    boxplot([postPnProfL(:,i),postPnProfR(:,i)],[zeros(1,length(postPnProfL(:,i))),ones(1,length(postPnProfR(:,i)))], 'notch', 'on')
...
subplot(1,2,1)
set(gcf, 'color', 'w')
hist(fractIpsiInputs, 20)
xlabel('contacts on ipsi PN/sum ipsi orn--> pn contacts')
ylabel('Freq')
title('Fraction of Ipsilateral ORN-->PN Inputs accounted for by each ORN')
xlim([0 .09])
ylim([0 20])
stdOutL=['std: ', num2str(std(fractIpsiInputs))];
cvOutL=['CV: ', num2str(std(fractIpsiInputs)/mean(fractIpsiInputs))];
text(.005,16.5,stdOutL)
text(.005,16,cvOutL)
...
subplot(1,2,2)
set(gcf, 'color', 'w')
hist(allSimFractIpsi, 20)
xlabel('contacts on ipsi PN/sum ipsi orn--> pn contacts')
ylabel('Freq')
title('Fraction of Ipsilateral ORN-->PN Inputs accounted for by each ORN')
xlim([0 .09])
ylim([0 300000])
stdOutL=['std: ', num2str(std(allSimFractIpsi))];
cvOutL=['CV: ', num2str(std(allSimFractIpsi)/mean(allSimFractIpsi))];
text(.005,210000,stdOutL)
text(.005,200000,cvOutL)
...
xlabel('Simulated Distribution Standard Deviations')
ylabel('Freq')
% xlim([0 15])
ylim([0 150])
line([std(fractIpsiInputs) std(fractIpsiInputs)], [0 150], 'color', 'r')  
text(.0095,125, ['Fract sim STDs >= real STD: ', num2str(sum(stdSimFractIpsi>=std(fractIpsiInputs))/r)])
